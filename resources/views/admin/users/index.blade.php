            ...
            <div class="row">
                <form action="{{url('admin/users/index')}}" method="post">
                    ...
                    <div class="col-lg-2">
                        ...
                        <button type="button" style="margin-left:5px;" class="btn btn-default" onclick="location.href='{{ url('admin/users/index') }}'">重置</button>
                        {{--@if(Gate::forUser(auth('admin')->user())->check('admin.users.add'))--}}
                            {{--<button type="button" style="margin-left:5px;" class="btn btn-default" onclick="location.href='{{url('admin/users/add')}}'"><i class="fa fa-plus-circle"></i>添加粉丝</button>--}}
                        {{--@endif--}}
                    </div>
                </form>
                        ...
                        <tr>
                            ...
                            <th data-sortable="false">
                                @if(Gate::forUser(auth('admin')->user())->check('admin.users.address'))
                                    <a href="{{url('admin/users/address',['uid'=>$v->id])}}" class="btn btn-success">
                                        <i class="fa"></i> 查看收货地址
                                    </a>
                                @endif
                                @if(Gate::forUser(auth('admin')->user())->check('admin.users.edit'))
                                    <a href="{{url('admin/users/edit',['id'=>$v->id])}}" class="btn btn-success">
                                        <i class="fa"></i> 编辑
                                    </a>
                                @endif
                                    @if(Gate::forUser(auth('admin')->user())->check('admin.users.changeStatus'))
                                        @if($v->status)
                                        <a href="{{url('admin/users/changeStatus',['id'=>$v->id,'status'=>$v->status])}}" class="btn btn-success">
                                            <i class="fa"></i>冻结
                                        </a>
                                            @else
                                            <a href="{{url('admin/users/changeStatus',['id'=>$v->id,'status'=>$v->status])}}" class="btn btn-success">
