...
@section('content')
    <div class="box box-info">
        ...
        <div class="box-body">
            <div class="row">
                <form action="{{url('admin/goods_spec/index')}}" method="post">
                    {{--<div class="col-lg-2">--}}
                    ...
                    <div class="col-lg-2">
                        ...
                        {{--<button type="button" style="margin-left:5px;" class="btn btn-default" onclick="location.href='{{ url('admin/goods_spec/index') }}'">重置</button>--}}
                        @if(Gate::forUser(auth('admin')->user())->check('admin.goods_cat.add'))
                            <button type="button" style="margin-left:5px;" class="btn btn-default" onclick="location.href='{{url('admin/goods_spec/add')}}'"><i class="fa fa-plus-circle"></i>添加规格</button>
                        @endif
                    </div>
                </form>
                ...
                <div class="box-body">
                    <table id="tags-table" class="table table-bordered table-hover">
                        ...
                        <tr>
                            ...
                            <th class="hidden-sm">规格名称</th>
                            <th class="hidden-sm">规格别名</th>
                        ...
                        @foreach($data as $k=>$v)
                        <tr>
                            <th data-sortable="false" class="hidden-sm">{{$v->spec_id}}</th>
                            <th class="hidden-sm">{{$v->spec_title}}</th>
                            <th class="hidden-sm">{{$v->spec_alias}}</th>
                            <th class="hidden-md">{{$v->status ? "正常":"禁用"}}</th>
                            <th class="hidden-md">{{date('Y-m-d H:i:s',$v->create_time)}}</th>
                            <th data-sortable="false">
                                @if(Gate::forUser(auth('admin')->user())->check('admin.goods_spec.edit'))
                                    <a href="{{url('admin/goods_spec/edit',['id'=>$v->spec_id])}}" class="btn btn-success">
                                        <i class="fa"></i> 编辑
                                    </a>
                                @endif
                                    {{--@if(Gate::forUser(auth('admin')->user())->check('admin.goods_spec.changeStatus'))--}}
                                        {{--@if($v->status)--}}
                                        {{--<a href="{{url('admin/goods_spec/changeStatus',['id'=>$v->spec_id,'status'=>$v->status])}}" class="btn btn-success">--}}
                                            {{--<i class="fa"></i>禁用--}}
                                        {{--</a>--}}
                                            {{--@else--}}
                                            {{--<a href="{{url('admin/goods_spec/changeStatus',['id'=>$v->spec_id,'status'=>$v->status])}}" class="btn btn-success">--}}
