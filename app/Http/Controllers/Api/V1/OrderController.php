...
class OrderController extends ApiBaseController
{
    ...
    public function writeOff(Request $request){
        ...
        if($result){
            ...
            return response()->json(['msg'=>Msg::getMsg(Msg::$err_failedWriteOff),'code'=>Msg::$err_failedWriteOff]);
        }
    }
    ...
    public function writeOffList(Request $request){
        ...
        $user = $this->usersModel->where(['id'=>$user_id,'is_businesser'=>1,'status'=>1])->first();
        if(empty($user)){//当前用户不是商家
            return response()->json(['msg'=>Msg::getMsg(Msg::$err_noUser),'code'=>Msg::$err_noUser]);
        }
        $list = $this->orderWriteOffModel
            ->join('orders','orders.order_id','orders_writeoff.order_id')
            ->join('users','users.id','orders.user_id')
            ->select('mobile','total_score','orders_writeoff.*')
            ->where(['orders_writeoff.user_id'=>$user_id])
            ->get()
            ->toArray();
        foreach($list as $k=>$v){
